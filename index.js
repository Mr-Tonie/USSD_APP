...
const accounts = {
  '+263771234567': { balance: 125.00, loanBalance: 320.00, nextPayment: '30 Oct 2025' },
};
...
app.post('/ussd', async (req, res) => {
  ...
  let response = '';
  const userAccount = accounts[phoneNumber] || { balance: 0, loanBalance: 0, nextPayment: 'N/A' };
...
We'll notify you once it's approved.`;
    ...
    case '2': // Check Loan Balance
      response = `END Your current loan balance is USD ${userAccount.loanBalance.toFixed(2)}.
...
2. Bank Transfer`;
    ...
    case '4': // Check Loan Status
      const loanStatus = await Loan.findOne({ phoneNumber }).sort({ dateApplied: -1 });
      if (loanStatus) {
...
3. Change PIN`;
        ...
        response = `END Your savings account balance is USD ${userAccount.balance.toFixed(2)}.`;
      } else if (textArray[1] === '2') {
